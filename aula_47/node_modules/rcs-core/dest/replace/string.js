...
const replaceString = (string, regex, options = {}) => {
  let result;
  let tempString = string; // save the string characters

  const beginChar = string.charAt(0);
  const endChar = string.charAt(string.length - 1); // remove the string characters

  tempString = tempString.slice(1, tempString.length - 1);
  let selectorLib = selectorsLibrary_1.default; // detect if it's a selector

  let surelySelector = tempString.match(regex_1.default.likelySelector) !== null;
  ...
  if (options.classOnly !== undefined) {
    surelySelector = false; // if we know that it's a class or an id, let's only search those

    selectorLib = options.classOnly ? selectorsLibrary_1.default.getClassSelector() : selectorsLibrary_1.default.getIdSelector();
  }

  const bastardSplitChars = ' =,()"\'\\]';
  let stringArray = tempString.split(new RegExp(`(?=[#.${bastardSplitChars}])`));
  let previousEqual = false;
  let previousAttr = ''; // replace every single entry
  ...
  stringArray = stringArray.map(element => {
    ...
    const startSplitChar = bastardSplitChars.indexOf(element.charAt(0)) === -1 ? '' : element.charAt(0);
    const tempElement = element.slice(startSplitChar.length, element.length);
    const startWithSelector = attributeLibrary_1.AttributeLibrary.isSelector(tempElement);
    prevEqual = prevEqual || startSplitChar === '='; // if we had "div[attr=something", react only if attr is a class, id or for
    // element will contain "div[attr" or " attr", we only care about the attribute name

    const match = !startWithSelector && prevEqual ? previousAttr.match(/[^a-zA-Z]+([a-zA-Z]+)/) : null;
    const previousAttributeName = match !== null ? match[1] : null;
    const isAttributeInteresting = previousAttributeName === 'class' || previousAttributeName === 'id' || previousAttributeName === 'for';
    previousAttr = interestingValue;

    if (!options.forceReplace && surelySelector && !startWithSelector && (!prevEqual || !isAttributeInteresting)) {
  ...
  result = stringArray.join(''); // add the string characters
